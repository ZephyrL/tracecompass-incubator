...
import org.eclipse.tracecompass.incubator.internal.jpftrace.core.analysis.spanlife.SpanLifeEntryModel.LogEvent;
...
public class SpanLifeDataProvider extends AbstractTimeGraphDataProvider<@NonNull SpanLifeAnalysis, @NonNull TimeGraphEntryModel> {
    ...
    // class ID 
    public static String ID = "org.eclipse.tracecompass.incubator.jpftrace.analysis.spanlife.dataprovider";
    ...
    public @NonNull TmfModelResponse<@NonNull Map<@NonNull String, @NonNull String>> fetchTooltip(Map<String, Object> fetchParameters, @Nullable IProgressMonitor monitor) {
        ...
        try {
            ...
            traceLogsQuark = ss.getQuarkRelative(ss.getQuarkAbsolute(traceId), IJpfTraceConstants.LOGS);
    ...
    private void addTrace(ITmfStateSystem ss, Builder<@NonNull TimeGraphEntryModel> builder, int quark, long parentId) {
        ...
        try {
            int logsQuark = ss.getQuarkRelative(quark, IJpfTraceConstants.LOGS);
            logsQuarks = ss.getSubAttributes(logsQuark, false);
        } catch (AttributeNotFoundException e) {
            logsQuarks = new ArrayList<>();
        }

        int jpfTraceSpansQuark;
        try {
            jpfTraceSpansQuark = ss.getQuarkRelative(quark, SpanLifeStateProvider.JPF_TRACE_ATTRIBUTE);
        } catch (AttributeNotFoundException e) {
            return;
        }
        ...
        try {
            ...
            addChildren(ss, builder, jpfTraceSpansQuark, traceQuarkId, logsQuarks);
            return;
        }
        addUstChildren(ss, builder, jpfTraceSpansQuark, ustSpansQuark, traceQuarkId, logsQuarks);
    }
    ...
    private void addChildren(ITmfStateSystem ss, Builder<@NonNull TimeGraphEntryModel> builder, int quark, long parentId, List<Integer> logsQuarks) {
        for (Integer child : ss.getSubAttributes(quark, false)) {
            ...
            if (!childName.equals(IJpfTraceConstants.LOGS)) {
    ...
    private void addUstChildren(ITmfStateSystem ss, Builder<@NonNull TimeGraphEntryModel> builder, int jpfTraceQuark, int ustQuark, long parentId, List<Integer> logsQuarks) {
        for (Integer child : ss.getSubAttributes(jpfTraceQuark, false)) {
            String childName = ss.getAttributeName(child);
