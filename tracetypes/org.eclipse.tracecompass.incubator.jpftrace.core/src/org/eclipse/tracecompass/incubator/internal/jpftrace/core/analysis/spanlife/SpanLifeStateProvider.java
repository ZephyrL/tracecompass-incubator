...
public class SpanLifeStateProvider extends AbstractTmfStateProvider {
     ...
     */
    public static final String JPF_TRACE_ATTRIBUTE = "jpfTraceSpans"; //$NON-NLS-1$
    ...
    public SpanLifeStateProvider(ITmfTrace trace) {
        ...
        fHandlers.put("JpfTraceSpan", this::handleSpan); //$NON-NLS-1$
    ...
    private void handleSpan(ITmfEvent event, ITmfStateSystemBuilder ss) {
        ...
        String traceId = event.getContent().getFieldValue(String.class, IJpfTraceConstants.TRACE_ID);
        int traceQuark = ss.getQuarkAbsoluteAndAdd(traceId);

        int jpfTraceSpansQuark = ss.getQuarkRelativeAndAdd(traceQuark, JPF_TRACE_ATTRIBUTE);

        Boolean errorTag = Boolean.parseBoolean(event.getContent().getFieldValue(String.class, IJpfTraceConstants.TAGS + "/error")); //$NON-NLS-1$
        String processName = event.getContent().getFieldValue(String.class, IJpfTraceConstants.PROCESS_NAME);

        int spanQuark;
        String name = String.valueOf(TmfTraceUtils.resolveAspectOfNameForEvent(event.getTrace(), "Name", event)); //$NON-NLS-1$
        String spanId = event.getContent().getFieldValue(String.class, IJpfTraceConstants.SPAN_ID);
        String refId = event.getContent().getFieldValue(String.class, IJpfTraceConstants.REFERENCES + "/CHILD_OF"); //$NON-NLS-1$
        if (refId == null) {
            spanQuark = ss.getQuarkRelativeAndAdd(jpfTraceSpansQuark, name + '/' + spanId + '/' + errorTag + '/' + processName);
        } else {
        ...
        Map<Long, Map<String, String>> logs = event.getContent().getFieldValue(Map.class, IJpfTraceConstants.LOGS);
        if (logs != null) {
            // We put all the logs in the state system under the LOGS attribute
            Integer logsQuark = ss.getQuarkRelativeAndAdd(traceQuark, IJpfTraceConstants.LOGS);
